...
def read_pdf_pypdf(file_path):
    ...
    try:
        ...
        return f"Error using pypdf: {error!s}"
...
def read_pdf(file_path):
    ...
    error_msg += (
        "Please install one of them: 'pip install pypdf' or 'brew install poppler'."
    )
    return error_msg
...
def main():
    if len(sys.argv) < 2:
        print("Usage: python read-pdf.py <pdf_path>", file=sys.stderr)
        sys.exit(1)
